    ...
    return (
        <div>
            ...
            <div className='row'>
                <div className='col-sm'>
                    ...
                    <select id="types" onChange={(e) => { setOkno(e.target.value); oknoD({ type: "OKNO", payload: e.target.value }) }} className="select-right-prof form-control custom-select">
                        <option key='1112' value="Одностворчатое окно" className="optionProfA">Одностворчатое окно</option>
                        <option key='1122' value="Двустворчатое окно" className="optionProfB">Двустворчатое окно</option>
                        <option key='1132' value="Трехстворчатое окно" className="optionProfC">Трехстворчатое окно</option>
                    </select>
                </div>
                <div className='col-sm'>
                    ...
                    <select id="prof" onChange={(e) => { setProfile(e.target.value); profiles({ type: "PROFILE", payload: e.target.value }) }} className="select-right-prof form-control custom-select">
                        <option key='111' value="Россия, Турция" className="optionProfA">Россия, Турция</option>
                        <option key='112' value="3-камерный" className="optionProfB">3-камерный</option>
                        <option key='113' value="5-камерный" className="optionProfC">5-камерный</option>
                    </select>
                </div>
                <div className='col-sm'>
                    ...
                    <select id="stv" onChange={(e) => { setStvorka(e.target.value); stvorkaD({ type: "STVORKA", payload: e.target.value }) }} className="select-right form-control custom-select">
                        ...
                        <option key='115' value="Поворотное" className="optionB">Поворотное</option>
                        <option key='116' value="Поворотно-откидное" className="optionC">Поворотно-откидное</option>
                    </select>
                </div>
            </div>
            ...
            <select onChange={(e) => gridRez({ type: "GRIDREZ", payload: e.target.value })} className="select-right-podok form-control custom-select" id="podok">
                ...
                {
                    grid.map((x) =>
                        <option key={x.grid_id + 2} value={x.white} className={"option-podok-white-" + x.grid_id}> Белый {x.size} мм.</option>

                    )
                }
                {
                    grid.map((p) =>
                        <option key={p.grid_id + 3} value={p.color} className={"option-podok-color-" + p.grid_id + 2}> Цветной {p.size} мм.</option>
            ...
            <select onChange={(e) => tideRez({ type: "TIDEREZ", payload: e.target.value })} className="select-right-otliv form-control custom-select" id="otliv">
                ...
                {
                    tide.map((white) =>
                        <option key={white.tide_id + 4} value={white.white} className={"option-podok-color-" + white.tide_id + 4}> Белый {white.size} мм.</option>
                    )
                }
                {
                    tide.map((color) =>
                        <option key={color.tide_id + 5} value={color.color} className={"option-podok-color-" + color.tide_id + 6}> Цветной {color.size} мм.</option>
                    )
                }
            </select>
            <label htmlFor="setka">Сетка</label>
            {dopp.map((x) => <div key={x.id}>
